...
1. `BeanFactory` + 국제화, 환경변수, 애플리케이션 이벤트, 리소스 조회
2. 정확히 스프링 컨테이너를 부를 때, `BeanFactory`와 `ApplicationContext`는 구분되는게 맞지만 `ApplicationContext`가 `BeanFactory`의 모든 기능을 다가지고 있기때문에 `BeanFactory`를 실질적으로 사용할 일은 없다.
...
4. 스프링 컨테이너 `Applicationcontext`에는 Bean 저장소가 있다.

- key(bean이름) - value(bean객체참조주소) 형식으로 저장

5. 기본적으로 `싱글톤`으로 관리가 된다.
